...
def gaussian_mixture_3d(batch_size, n_dim=3, n_labels=10, x_var=0.5, y_var=0.1, z_var=0.1, label_indices=None):
    if n_dim != 3:
        raise Exception("n_dim must be 3.")
    ...
    def sample(x, y, label, n_labels):
        ...
        new_y += shift * sin(r)
        z = 11 * random.random()
        return np.array([new_x, new_y, z]).reshape((3,))

    x = np.random.normal(0, x_var, (batch_size, (int)(n_dim/3)))
    y = np.random.normal(0, y_var, (batch_size, (int)(n_dim/3)))
    z = np.empty((batch_size, n_dim), dtype=np.float32)
    for batch in range(batch_size):
        for zi in range((int)(n_dim/3)):
            if label_indices is not None:
                z[batch, zi*3:zi*3+3] = sample(x[batch, zi], y[batch, zi], label_indices[batch], n_labels)
            else:
                z[batch, zi*3:zi*3+3] = sample(x[batch, zi], y[batch, zi], np.random.randint(0, n_labels), n_labels)
...
def swiss_roll_3d(batch_size, n_dim=3, n_labels=10, label_indices=None):
    if n_dim != 3:
        raise Exception("n_dim must be 3.")
    ...
    for batch in range(batch_size):
        for zi in range((int)(n_dim/3)):
            if label_indices is not None:
                z[batch, zi*3:zi*3+3] = sample(label_indices[batch], n_labels)
            else:
                z[batch, zi*3:zi*3+3] = sample(np.random.randint(0, n_labels), n_labels)
    return z